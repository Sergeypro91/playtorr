...
export class UserService {
  ...
  async editUser(editingUser: UserSession, editableUser: EditUserDto) {
    ...
    if (editableUser.tgId) {
      const existingTgIdUser = await this.findUserByTgId(editableUser.tgId);

      if (existingTgIdUser && existingTgIdUser.email !== editableUser.email) {
  ...
  async deleteUsers(users: string[]) {
    return await this.userModel.deleteMany({ email: { $in: users } }).exec();
  }
}
